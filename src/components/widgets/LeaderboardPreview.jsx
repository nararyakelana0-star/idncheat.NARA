...
/* =====================================================================
   Leaderboard Preview — Top 5 mingguan (widget dashboard)
   ===================================================================== */

export default function LeaderboardPreview() {
  const { navigate } = useApp()
  const { user } = useAuth()
  const top5 = LEADERBOARD.mingguan.slice(0, 5)
    ...
    <div className="card flex h-full flex-col p-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-display font-bold text-slate-900 dark:text-white">Top 5 Minggu Ini</p>
            <p className="text-xs text-slate-500 dark:text-slate-400">Berdasarkan XP mingguan</p>
          </div>
        </div>
        <button
      ...
      <ul className="mt-4 flex-1 space-y-1">
        {top5.map((p) => (
          ...
          >
            ...
            <span
              className={`grid h-8 w-8 shrink-0 place-items-center rounded-full bg-gradient-to-br text-[11px] font-extrabold text-white ${avatarGradient(p.name)}`}
            >
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="block text-[11px] text-slate-400">{p.prog}</span>
            </span>
